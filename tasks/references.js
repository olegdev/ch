...
module.exports = function(grunt) {
	return function() {
	    ...
	    if (!fs.existsSync(options.dictionaryDest + '/' + dictionary.defaultLang)) {
	    	fs.mkdirSync(options.dictionaryDest + '/' + dictionary.defaultLang);
	    }	    
	    fs.writeFileSync(options.dictionaryDest + '/' + dictionary.defaultLang + '/' + referenceFileName, JSON.stringify(translates), {encoding: 'utf8'});
		...
		fs.writeFileSync(options.dest + '/' + dictionary.defaultLang + '/' + referenceFileName.replace('.json', '.js'), fileStr, {encoding: 'utf8'}); // и кладу в нужную папку под тем же именем
	}
};